...
Monomial21100:=function(k,identify,m)
  local chars,chi,h,lambda,restricted,candidates,roots,rows,values,j,classes,x;
  chars:=IrrConlon(k); roots:=List([0..m-1],j->E(m)^j); rows:=[];
  for chi in chars do
    if chi[1]=1 then
      h:=k; lambda:=chi;
    else
  ...
  od;
  if Length(chars)<>Length(Irr(k)) then Error("Conlon completeness"); fi;
  return rows;
end;;
