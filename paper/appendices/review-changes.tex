...
The introduction now also mentions the parallel Fable experiment by the
same organizers under the same local conditions. A new
Appendix~\ref{app:parallel-fable}, after the historical correspondence,
provides a preliminary review and comparison, with a pinned source
snapshot and three targeted computational checks. It records overlap,
prior-work status and complementary results, and sharpens the threshold
in Fable's construction for 2.78 from 86 to 78. This later comparison
does not alter either experiment's historical result count.

\input{appendices/v2-changes}

\input{appendices/v3-changes}

\input{appendices/v4-changes}
