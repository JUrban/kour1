...
These notes record reading in the manuscript phase, separately from the
dated literature audits in the frozen archive. Reading a statement for an
application is not a full audit of its proof or a comprehensive novelty search.

## Experimental and related-work sources

- Notebook: local title pages and the final ledger's transcribed statements,
  tied to the input PDF digest. The editors' current site and download were
  located; the evolving public URL does not fix the input revision.
- GPT-6 Astra, prompt caching, reasoning: official OpenAI documentation
  opened on 14 September 2026. Used for terminology and subcounter meaning,
  not to infer the historical model, invoice, or actual remote hardware.
- FunSearch: primary publisher search content and publisher-supplied
  Crossref metadata. Direct Nature opening failed; PMC returned a browser
  check. The manuscript makes only the supported high-level comparison
  about executable evaluation. No detailed benchmark comparison is claimed.
- AlphaProof: the primary Nature article was opened and its formal Lean
  environment, proof network/tree-search description and bibliographic
  metadata read. It appeared online in November 2025 but is in volume 651,
  pages 607–613 (2026); the bibliography distinguishes these dates.
- First Proof and First Proof Second Batch: primary arXiv abstracts,
  author lists, submission dates and descriptions of the evaluation setting
  read. No numerical comparison with this experiment is made.

## Mathematical sources read or rechecked

- Conte–Petschick, DOI 10.1007/s00605-025-02127-5: retained published PDF
  opening pages and current primary publisher text, including Question 1,
  Theorem 1.1, the parameter-free convention and existential restriction.
  Bibliography now uses volume 209, pages 215–240 (2026), and records the
  October 2025 online date. The linked correction,
  DOI 10.1007/s00605-025-02129-3, was opened: it corrects an author-name
  concatenation, not the mathematics cited here.
- Kida: retained published opening pages establish author, title,
  journal 28 (2025), 697–712. Conjecture 1.3 and Example 5.5 reread:
  the order-96 group contains SL2(3). The example alone is not the
  manuscript's nonmonomial counterexample.
- Tsang: retained accepted-version opening pages and arXiv metadata
  establish the normalizer definitions and published reference,
  JPAA 229 (2025), 107843, DOI 10.1016/j.jpaa.2024.107843.
  Theorem 4.5, its exceptions, and the following discussion reread.
- Shakhova, dominion lattices: primary MathNet metadata and retained Russian
  original p.239 read. It explicitly extends the definition to a domain
  outside the target quasivariety. This is the convention used for the
  smaller 16.20 construction. Its later abelian classification is not used.
- Plotkin–Gvaramia: primary arXiv author/title record checked. Section 1.1
  of the retained preprint reread: a fixed commutative ring with identity
  is used. Conversion between left and right action conventions is harmless.
- HNN: standard named embedding/normal-form theorem used. The DOI route
  failed to open during this phase; the explicit tensor/tree module argument
  is new exposition from the retained proof, not a quotation from that source.
- Bergman: primary author PDF opened, Section 1 read. It explicitly states
  the two-sided-order Mal'cev–Neumann theorem and the inclusion of the
  finite-support group algebra. Its later results about right orders
  are not used.
- Sakarovitch: primary author-hosted chapter opened, Theorems 21–22 read.
  They supply bi-orderability of a finite direct product of free groups
  and the series division ring. The bibliography identifies the actual
  dated author manuscript rather than inventing chapter pagination.
- Fine–Gaglione–Rosenberger–Spellman: retained preprint Theorem 3.4(1)
  and Lemma 3.6 read, primary arXiv metadata checked. Used only to credit
  the characteristic-two centralizer fact. Other claims in that paper,
  particularly about characteristic zero, are not imported.
- Johnston–Rumynin: retained arXiv v2, Conjecture 9, the filtration
  definition, Proposition 3.3 and its Ext display reread; current v2
  metadata checked. The manuscript specifies the version for its conflict
  with the explicit order-twelve construction. The C2 extension calculation
  is provided directly. Full publication comparison remains part of the
  final source review.

- Modular Atlas A7mod5.pdf, blocks 9--10: primary two-page table opened;
  its five-by-four decomposition matrix agrees with the exposition.
  GAP Character Table Library information for 3.A7 opened: order 7560,
  perfectness and table provenance checked. Final review still must
  recheck the exact Galois/Frobenius assignments against the retained data.
- Reingold–Vadhan–Wigderson: retained definition and metadata read.
  Cavaleri–D'Angeli–Donno: Lemma 3.5 and its matrix-factorization proof read,
  primary arXiv metadata checked. Used to credit the finite-factor method.
- Dyer: AMS route returned 403; its deletion theorem was read as stated
  and explicitly attributed in Marco Lotz's primary arXiv:2406.05218v1,
  Theorem 1.20. The manuscript proves the special free-product case itself.
- Mal'cev 1961: primary MathNet PDF opened, title, pages and introductory
  finite-group-theory undecidability statement read. Its coding proof is
  imported as the standard theorem, not claimed independently audited.
- Bastos–Monetta: Proposition 18 and the adjoining minimal-simple-group
  setting reread; primary arXiv metadata checked. Monakhov's abstract and
  stated primary-commutator criterion read. BCGLO published metadata and
  hyperfocal formula checked; exact initial theorem location remains for
  final source review. Guralnick–Malle author preprint opening classical
  Baer–Suzuki theorem read; later variations are not used.
- Lazard: retained pp.179–180, subgroup paragraph and Theorem 4.6 reread.
  Primary Numdam bibliography checked. Trappeniers v3 Proposition 5.5
  with residual-correction argument and all relevant Section 6 statements
  reread. The strong-nilpotence restriction after Proposition 6.2 is
  explicit; the manuscript obtains its polynomial inverse directly.
- Protasov 1986: retained E-topology definition and Lemma 2 reread,
  with the preceding Komarov–Protasov setup. The compact-family proof
  is included in full. Dow–Watson Example 1 reread; primary DML metadata
  checked. The constructed non-Skula space is expressly prior work.
- Bardakov: retained publication year and original problem context
  checked. CGG published author list and normalized-dual claim locations
  checked; full claims and rendering remain for final review. The
  Hemkemeier–Vallentin retained revision identifies Kneser's argument
  in its abstract; it is credited as context, not imported as a proof input.
- Gvaramiya and Krapež–Marinković: retained sources support the ordinary
  quasigroup interpretation. Primary DML metadata checked; Gvaramiya's
  DOI route failed in this phase. Final review must reread the precise
  Section 3 conventions and theorem statements.
- Amelio v2: current arXiv version record checked. Retained Definition 4.46,
  full Proposition 6.1, Remark 6.6, Theorem 6.7 and its initialization/
  selection step, and Remark 6.8 reread. The selected odd relation uses
  the construction, not just the headline theorem. Deeper cone-off
  estimates remain an imported dependency.
- Song–Xu–Ye v2: primary metadata and retained Propositions 1.4 and 2.1
  read; the latter expressly credits Boccara for the three-partition case.
  Brenner–Evans–Silberger Lemma 5 and Rosser–Schoenfeld Theorems 4 and 9
  reread from retained primary PDFs/text. Their respective hypotheses
  and strict prime-estimate inequalities match the manuscript.

## Remaining work

Add precise primary-source records for the later candidate and partial
