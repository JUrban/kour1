\section{Detailed chronology and computation record}
\label{app:chronology}

The chronology has two complementary sources. Commits give dated,
recoverable states of the repository. The timestamped session messages give an account of what the agent believed it had established at each
...
\caption{Repository and recorded tool activity during the research window.
The upper panel counts the 198 commits within the research window;
the closeout commit 16 seconds later is excluded.
The middle panel retrospectively follows the first committed principal
proof file for each family retained on the final candidate list.
It excludes withdrawn candidates and does not measure the contemporaneous
candidate count, proof completion, novelty, or outside acceptance.
The lower panel counts completed command/polling, web, file-change
and image-view observations in one-hour bins. It excludes context
compaction and sleep operations; counts are not independent experiments.
All panels are derived from the supplied CSV measurements.}
...
(10 September, 21:00:51 UTC) announced a possible formula selecting generators of the infinite cyclic
center of the integral Heisenberg group; message~152 reported a written
candidate and local commit at 21:06:04 UTC. Section~\ref{subsec:concise} gives the
...
discovery ability. The mathematical sections identify these dependencies.
...
verification remained outstanding at 11 September, 11:08:29 UTC.
Six GAP processes subsequently
...
The last workers were still active on 12 September at 12:45 UTC.
Message~28901, at 13:10:31 UTC, reports all six workers finished with
zero exits and a successful aggregate check, while explicitly retaining
a discrepant exit from the historical launcher. Message~29167 reports
the committed proof packet at 13:31:41 UTC. These dated observations
connect the long-running process records to the final mathematical claim.
