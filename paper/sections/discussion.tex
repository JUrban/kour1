...
The retained export limits behavioral reconstruction. It lacks raw
tool-call bodies, internal-analysis events, complete human-message
history, and per-event wall-clock times. Repository artifacts supply
many commands and outcomes, but they do not turn the export into a
complete interaction trace. The model setting is human-supplied
metadata; the renderer version is observed in the export. Remote
inference hardware, actual peak local resource consumption, provider
invoices, and a complete historical sampling configuration are
unavailable. Reported tokens and elapsed time should not be converted
into an audited monetary or energy cost.
...
substitutes for specialist assessment. Authorship and responsibility
for any submitted paper belong to the human authors; the use of the
agent in research and manuscript preparation should remain explicit.
...
along with local CPU time and peak memory. Keep reported usage counters
and billing records separate until reconciled.
