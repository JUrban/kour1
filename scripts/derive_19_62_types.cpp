...
constexpr int MAXROOTS=48,INF=64;
int ROOTS=0;
using Values=std::array<std::array<uint8_t,2>,MAXROOTS>;
struct Rule {int r,s,k,i,j,c,index,v2,v3;};
...
std::array<int,2> derive(const std::vector<Rule>&rules,int root,
    ...
    std::vector<std::array<std::array<Edge,2>,MAXROOTS>> witness(count);
                ...
                if(d==n){
                    ...
                    for(auto r:entry.second){
                        for(int prime=0;prime<2;++prime){
                            int cost=(prime==0?r.v2:r.v3)+i*values[a][r.r][prime]+j*values[b][r.s][prime];
...
int main(int argc,char **argv){
    ...
    std::ifstream in(argv[1]);int nr;in>>ROOTS>>nr;
    if(ROOTS<1||ROOTS>MAXROOTS)throw std::runtime_error("root count");
    std::vector<Rule>rules(nr);
    for(int k=0;k<nr;++k){auto &r=rules[k];in>>r.r>>r.s>>r.k>>r.i>>r.j>>r.c;r.index=k;r.v2=valuation(r.c,2);r.v3=valuation(r.c,3);}
