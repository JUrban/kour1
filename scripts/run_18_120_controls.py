...
for label,cmd,files,sentinel in [
 ('export',['gap-4.16.1/gap','-l',str(ROOT/'gap-4.16.1'),'-q','-b','-T','-m','128m','-o','1g','scripts/export_18_120_controls.g'],['scripts/export_18_120_controls.g','results/18.120-control-inputs.json'],'PASS_18120_EXPORT'),
 ('controls',[sys.executable,'scripts/check_18_120.py'],['scripts/check_18_120.py','results/18.120-controls.json'],'PASS_18120_CONTROLS')]:
 log=f'results/18.120-{label}.log';start=time.monotonic()
 ...
 runs.append(r);(ROOT/'results/18.120-controls-process.json').write_text(json.dumps({'runs':runs},indent=2)+'\n');print(json.dumps(r),flush=True)
 assert p.returncode==0 and r['clean_log'] and r['sentinel']
print('PASS_18120_CONTROL_RUNNER')
