...
def main():
    ...
    parser.add_argument('order', type=int, choices=[96,1536])
    parser.add_argument('--shards', type=int, default=8)
    parser.add_argument('--tag', default='')
    args = parser.parse_args()
    assert 1 <= args.shards <= 8
    n = args.order
    first,last,total = {96:(1,231,231),1536:(408526598,408544625,408641062)}[n]
    count = last-first+1
    assert re.fullmatch(r'[a-z0-9-]*',args.tag)
    prefix = ROOT / (f'results/20.21-order{n}-interval'+('-'+args.tag if args.tag else ''))
    script = ROOT / 'scripts/search_20_21_interval.g'
    ...
    state = dict(status='RUNNING',order=n,total_groups=total,interval_first=first,interval_last=last,interval_count=count,shards=args.shards,
    ...
    try:
        for i in range(args.shards):
            ...
            code = (f'SearchOrder2021:={n};SearchIndex2021:={i};'
                    ...
                    f'SearchFirst2021:={first};SearchLast2021:={last};')
    ...
    for job in state['jobs']:
        ...
        for nn,index,cs,aa in rows:
            assert nn == n and first <= index <= last and (index-first)%args.shards == i
