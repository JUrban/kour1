...
def direct_product_series(family,mode,e,n):
    """Independent generalized-binomial expansion, with Laurent exponents.

    A base is 1+O(t^(2s)), and its exponent has pole order <=s.
    The jth binomial term has order >=sj, so j<=n/s suffices;
    ordinary base powers need only be retained through degree 2n.
    """
    size=2*n
    if mode=='C':
        a=one(size); b=one(size)
        for k in range(2,size+1): a[k]=Q((-1)**(k-1)); b[k]=Q(1)
    elif mode=='RS':
        a=one(size); b=one(size)
        for k in range(2,size+1): a[k]=Q(2*(-1)**(k-1))
    else:
        # Invert a product of denominator polynomials, independently of
        # reciprocal_sum's successive geometric-division recurrence.
        bases=[]
        for kind in ('U','L'):
            f=one(size)
            for m in range(1,size+1):
                ...
                for i in range(1,m+1):
                    factor=one(size-m*m)
                    if i<=size-m*m: factor[i]=-((-1)**i if kind=='U' else 1)
                    den=mul(den,factor,size-m*m)
                inv=one(size-m*m)
                for k in range(1,len(inv)):
                    inv[k]=-sum(den[j]*inv[k-j] for j in range(1,k+1))
                for k,v in enumerate(inv): f[k+m*m]+=v
            bases.append(mul(f,[Q(1),Q(-1)],size))
        a,b=bases
    out=one(n)
    for d in range(1,n+1):
        kinds=[('N',d,a),('M',2*d,b)] if family=='U' else [('J',d,a),('K',d,b)]
        for kind,step,base in kinds:
            if kind=='N' and d%2==0: continue
            poly=counts(d,kind) if family=='U' else star_counts(d,kind,e)
            p={-k:v for k,v in poly.items()}
            h=[Q(0)]*(size+1)
            for k in range(1,size//step+1): h[k*step]=base[k]
            hpow=one(size); choose={0:Q(1)}; factor=one(n)
            for j in range(1,n//step+1):
                hpow=mul(hpow,h,size)
                pj=p.copy(); pj[0]=pj.get(0,Q(0))-(j-1)
                nxt={}
                for k,v in choose.items():
                    for l,w in pj.items(): nxt[k+l]=nxt.get(k+l,Q(0))+v*w/j
                choose={k:v for k,v in nxt.items() if v}
                for k,v in choose.items():
                    for l in range(max(0,-k),min(size,n-k)+1):
                        factor[k+l]+=v*hpow[l]
            out=mul(out,factor,n)
    if family=='U': return mul(out,[Q(1),Q(1)],n)
    if mode=='RS': return mul(out,power([Q(1),Q(-1)],e,n),n)
    if mode=='SS':
        return mul(out,power(mul([Q(1),Q(-1)],reciprocal_sum('P',n),n),e,n),n)
    return out
...
def main():
    ...
    for key,row in PUBLISHED.items():
        for degree,(actual,published) in enumerate(zip(series[key],row)):
            if actual!=published:
                mismatches.append(dict(row=key,degree=degree,computed=str(actual),published=str(published)))
    assert mismatches==[dict(row='Sp2_SS',degree=9,computed='-193',published='-195')],mismatches
    # A second implementation uses finite generalized-binomial expansions.
    for mode in ('C','SS','RS'):
        for family,e in [('U',0),('Sp',1),('Sp',2)]:
            assert product_series(family,mode,e,12)==direct_product_series(family,mode,e,12)
    # A third check uses only the table's odd orthogonal row and the exact
    # relation SS(Sp;odd)=SS(Oodd;odd)*P/Q from Theorem 3.1.18.
    p=reciprocal_sum('P',9); q=reciprocal_sum('Q',9)
    orth_times_p=mul(list(map(Q,PUBLISHED['Oodd2_SS'])),p,9)
    quotient=[Q(0)]*10
    for k in range(10): quotient[k]=orth_times_p[k]-sum(q[j]*quotient[k-j] for j in range(1,k+1))
    assert quotient==series['Sp2_SS'][:10] and quotient[9]==-193
    ...
    artifact=dict(status='PASS',series_degree=DEGREE,
        ...
        published_coefficients_matched=10*len(PUBLISHED)-len(mismatches),
        published_discrepancies=mismatches,independent_binomial_checks=9*13,
        orthogonal_table_identity_coefficient=str(quotient[9]),
