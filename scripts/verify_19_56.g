...
(function()
 ...
 Check:=function(G,name)
  ...
  prime:=List(orders,n->Filtered(Set(FactorsInt(n)),IsPrimeInt));
  ...
  od;
  hyps:=[];
  for p in Filtered(Set(FactorsInt(Size(G))),IsPrimeInt) do
   P:=SylowSubgroup(G,p);
  ...
  for a in stars do for b in stars do
   ...
   if Gcd(oa,ob)=1 and Order(a*b)<oa*ob then
    good:=false;
    if oa>1 and ob>1 and Length(Set(FactorsInt(oa)))=1 and Length(Set(FactorsInt(ob)))=1 then
     primarygood:=false;
    fi;
 ...
 for q in [5,7,9] do
  ...
  abar:=Image(pi,w);
  g:=First(Elements(G),g->Length(Difference(Filtered(Set(FactorsInt(Order(abar*Image(pi,w^g)))),IsPrimeInt),[2]))>0);
  Assert(0,g<>fail);
