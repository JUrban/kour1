...
def nonnormal_sink():
    ...
    conjugates={mul(mul(inverse[g],x),g) for x in sink for g in gg}
    assert len(conjugates)==5
    # The union of the two axes has five elements. Its generated subgroup
    # has nine; replacing generation by a union was caught by the first run.
    normal_closure={ident()};todo=[ident()]
    for x in todo:
        for y in conjugates:
            z=mul(x,y)
            if z not in normal_closure:
                normal_closure.add(z);todo.append(z)
    ...
    return dict(group_order=72,sink_size=3,sink_group_order=3,normal_closure_order=9,
                conjugate_union_size=5,product_checks=5184)
...
def main():
    summary=dict(status='PASS',filtration=filtration(),mixed_rows=mixed(),
                 ...
                 rejected_shortcuts=['wrong_diagonal_pair','reversed_filtration',
                                     ...
                                     'union_of_conjugates_is_subgroup'])
