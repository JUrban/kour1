...
class Cyclotomic:
    def __init__(self, m):
        ...
        while t > 1 and t % p == 0:
            t //= p
        assert t == 1 and m >= p
        self.m, self.u, self.d = m, m//p, (p-1)*m//p
        self.zero = (0,)*self.d
        self.roots = []
        for i in range(m):
            row = [0]*self.d
...
def verify_case(row):
    ...
    ar,order = row['action_prime'],row['action_order']
    assert ar >= 2 and all(ar % k for k in range(2,ar)) and gcd(order,group.n) == 1
    t = order
    while t > 1 and t % ar == 0:
        t //= ar
    assert t == 1 and order > 1
    assert sorted(auto) == list(range(group.n)) and any(auto[i] != i for i in range(group.n))
    power = list(range(group.n))
    for j in range(1,order+1):
        power = [auto[x] for x in power]
        if j == order//ar:
            assert any(power[i] != i for i in range(group.n))
    assert power == list(range(group.n))
    ...
    for degree,values in chars:
        ...
        odd = [i for i,x in enumerate(mult) if x % ar]
        assert len(odd) == 1
        assert odd[0] not in correspondents
        correspondents.add(odd[0])
    ...
    assert correspondents == set(range(len(cchars)))
    assert len(summary) == len(cchars) and never_count == cab
    stats['groups'] += 1
    return dict(catalogue_id=row['catalogue_id'],action_order=order,action_prime=ar,
                fixed_order=len(fixed),fixed_abelianization=cab,
                characters=summary),stats


def main():
    data = json.loads((ROOT/'results/21.100-independent-certificate.json').read_text())
    expected = {(243,51,2),(243,56,2),(729,138,2),(729,245,2),(729,258,2),(729,268,2),
                (729,402,2),(256,10331,3),(256,56065,5),(256,53054,7),
                (2187,8007,4),(2187,8007,8),(2187,9276,13)}
    assert {(*r['catalogue_id'],r['action_order']) for r in data['groups']} == expected
    ...
    for row in data['groups']:
        ...
        print('VERIFIED',row['catalogue_id'],row['action_order'],dict(sorted(stats.items())),flush=True)
    rejected = []
    ...
    print('PASS_21100_INDEPENDENT models=13 irreducibles=%d invariant=%d nonlinear=%d mutations=5' %
