...
def check(data):
    ...
    pilot=json.loads((ROOT/'results/16.45-extension.json').read_text())
...
def run():return [check(d) for d in json.loads((ROOT/'results/16.45-extension-control-inputs.json').read_text())]

if __name__=='__main__':
    rows=run()
    (ROOT/'results/16.45-extension-controls.json').write_text(json.dumps(rows,indent=2)+'\n')
    print('PASS_1645_EXTENSION_CONTROLS',len(rows),'groups',sum(r['subgroups'] for r in rows),'subgroups',sum(r['associativity_triples'] for r in rows),'associativity triples')
