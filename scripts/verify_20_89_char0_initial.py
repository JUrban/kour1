...
def mul(x,y):
    ...
    return (u+s*a,v+s*b,z+s*s*c+u*s*b,k+j,pmul(p,q))
def inv(x):
    u,v,z,k,p=x;s=Q(2)**(-k)
    return (-s*u,-s*v,s*s*(u*v-z),-k,pinv(p))
...
summary=dict(pairs=720,commutators=2160,excluded_dilations=orbit_rows,s4_sink_histogram=hist,s4_sink_union=len(union),nonsplit_checks=nonsplit,nonsplit_torsion=torsion,central_torsion_orders=central)
