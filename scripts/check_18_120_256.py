#!/usr/bin/env python3
"""Independent table reconstruction of all factorization tests in two groups."""
import json,time
...
def check(data):
 ...
 row=next(r for r in json.loads((ROOT/'results/18.120-256.json').read_text())['rows'] if (r['order'],r['id'])==(n,data['id']))
...
def run():return [check(d) for d in json.loads((ROOT/'results/18.120-256-control-inputs.json').read_text())]
if __name__=='__main__':
 start=time.monotonic();rows=run();(ROOT/'results/18.120-256-controls.json').write_text(json.dumps(rows,indent=2)+'\n')
 print('PASS_18120_256_CONTROLS',len(rows),'groups',sum(r['factorization_representatives'] for r in rows),'representatives',sum(r['actual_factorizations'] for r in rows),'actual factorizations',time.monotonic()-start,'seconds')
