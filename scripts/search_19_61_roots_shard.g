...
Search1961:=function()
local nopts,m,contains,idx,row,gens,r,t,H,closure,params,p,edge,bad,expanded,hits,out,counts,tested,last;
...
expanded:=0;hits:=0;counts:=[];tested:=0;
last:=Length(Carpets1961)-((Length(Carpets1961)-ShardIndex1961-1) mod ShardCount1961);
for idx in [ShardIndex1961+1,ShardIndex1961+1+ShardCount1961..last] do
  tested:=tested+1;
  row:=Carpets1961[idx];
  ...
  if tested mod 100=0 then Print("PROGRESS_1961 ",RingLabel1961," ",[tested,idx,expanded,hits],"\n");fi;
od;
CloseStream(out);
Print("PASS_1961_SEARCH ",RingLabel1961," ",[tested,expanded,hits],"\n");
end;
Search1961();
QUIT;
