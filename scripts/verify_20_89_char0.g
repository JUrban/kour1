...
Matrix7:=function(u,v,z,k,p)
    ...
    a[1][1]:=8^k;a[1][2]:=u*4^k;a[1][3]:=z;
    a[2][2]:=4^k;a[2][3]:=v;a[3][3]:=1;
    for j in [1..4] do a[j+3][p[j]+3]:=1;od;
    return a;
end;;
Encode2089C:=function(a)
    local p;
...
for i in [0..719] do
    ...
    row:=[];
    for j in [1..3] do x:=Comm(x,a);Add(row,Encode2089C(x));od;
    if row[2][3][1]<>0 then nonzeroSecond:=nonzeroSecond+1;fi;
    if x{[1..3]}{[1..3]}<>IdentityMat(3,Rationals) or SignPerm(PermList(row[3][4]))<>1 then Error("common kernel");fi;
    Print("ROW2089C ",[i,row],"\n");
od;
if nonzeroSecond=0 then Error("missing class-two control");fi;
orbitrows:=[];;
...
for r in [1..8] do
    ...
    Add(central,Order(z));
od;
Print("SUMMARY2089C {\"pairs\":720,\"nonzero_second_commutators\":",nonzeroSecond,",\"commutators\":2160,\"excluded_dilations\":",orbitrows,",\"s4_sink_histogram\":",hist,",\"s4_sink_union\":",Length(all),",\"nonsplit_checks\":",nonsplit,",\"nonsplit_torsion\":",torsion,",\"central_torsion_orders\":",central,"}\n");
