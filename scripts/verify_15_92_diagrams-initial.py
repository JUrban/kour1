...
def add_rect(D,prefix,mode='handle'):
    D.tri(prefix+'l','right',prefix+'r')
    D.tri(prefix+'r','left',prefix+'l')
...
def basic(which):
    ...
    if which=='S':
        ...
        for i,port in enumerate(('U','R','D')):
            D.edge((str(i)+'l','R'),('ol',port))
            D.edge((str(i)+'r','L'),('orr','L' if port=='R' else port))
        ...
        for prefix,port in [('0','U'),('2','D')]:
            D.edge((prefix+'l','R'),('ol',port))
            D.edge((prefix+'r','L'),('orr',port))
...
def main():
    ...
    lines=['SetInfoLevel(InfoWarning,0);;']
    for a in actions:
        for gen in ('x','y','t'):
        ...
        if a['label'].startswith('chain'):
            lines += ['v:=(x*y*x*y^-1)^32760;;',
             ...
             'if Size(g)<>Factorial(n)/2 then Error("not alternating"); fi;',
