...
local n,i,G,ab,ns,k,q,cs,as,pair,tested,eligible,both,iso,idsC,idsA,out,hits,last,first;
n:=SearchOrder2021;
...
first:=SearchFirst2021+SearchIndex2021;
last:=SearchLast2021-((SearchLast2021-first) mod SearchShards2021);
for i in [first,first+SearchShards2021..last] do
  G:=SmallGroup(n,i);tested:=tested+1;
  if tested mod 500=0 then
    Print("PROGRESS_2021_SHARD ",[n,SearchIndex2021,tested,i,eligible,both,hits],"\n");
  fi;
  if n=1536 then
    Assert(0,IsNormal(G,SylowSubgroup(G,2)));
    Assert(0,not IsNormal(G,SylowSubgroup(G,3)));
  fi;
