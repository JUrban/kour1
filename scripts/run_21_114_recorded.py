#!/usr/bin/env python3
"""Run a named 21.114 stage and retain actual process status and output hashes."""
import argparse
import datetime
import hashlib
import json
...
def main():
    ...
    base = ROOT / ('results/21.114-' + args.name)
    ...
    record.update(ended_utc=now(), elapsed_seconds=time.monotonic()-start,
                  actual_returncode=rc, stdout_sha256=hashlib.sha256(stdout).hexdigest(),
