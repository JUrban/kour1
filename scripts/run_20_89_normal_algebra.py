#!/usr/bin/env python3
"""Record actual verifier exit status and preserve exact output bytes."""
...
stem=root/('results/20.89-normal-algebra-'+name)
...
accepted=code==0 and not err and out.count(b'PASS_20_89_NORMAL_ALGEBRA_')==1
...
print(json.dumps(record,sort_keys=True));print(out.decode(errors='replace')[-1800:]);print(err.decode(errors='replace'))
