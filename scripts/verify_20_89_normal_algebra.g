...
a:=fail;;identity:=fail;;field:=fail;;one:=fail;;e0:=fail;;
for p in [2,3,5,7] do
...
p:=7;;one:=One(GF(p));;a:=[[0,1],[1,0]]*one;;b:=[[0,-1],[1,-1]]*one;;
small:=Group(a,b);;e:=Set(Elements(small),x->Comm(x,a));;
if e<>Set([One(small),b,b^2]) or Comm(b,a)<>b then Error("normality control sink");fi;
c:=[[1,2],[2,1]]*one;;bad:=b^c;;
if bad in small then Error("missing failure of normalization");fi;
encoded:=List(e,Encode2089A);;Sort(encoded);
Print("CONTROL2089A [7,",encoded,",5,",Encode2089A(bad),"]\n");
