...
- **6.47 partial:** A self-contained proof covers all class-two groups. A 52-file pilot packet replays 289 binary word functions and 522,952 associativity triples, settling the word-operation question for five specific finite groups; one group exceeds the enumeration cap. The higher-class extension is running separately. Cooper1974 and Płonka2013 are explicit prior sources; no new count. See `research/6.47-class-two.md` and `research/6.47-pilot-report.md`.
- **5.25 partial:** Nontrivial bi-orderable metabelian groups have nonperiodic abelianization by the finite-order conjugation argument. The soluble case is not established; see `research/5.25-exploration.md`.
...
- **20.100 n=7 verifying:** generation completed42,891,332 nodes and786,577,344 edges with exit0. Full gzip integrity passes; six GAP workers started11:12 UTC with14GiB workspace ceilings each. All six are verified live; no n=7 proof is claimed until every shard and aggregate pass.
...
2. Continue novelty and mathematical audits of all thirty-four candidate resolutions.
