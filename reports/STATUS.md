...
Updated: 2026-09-11T00:21:58.405270+00:00.

- Active phase: proof audit, exact searches, and broader problem triage.
- Complete candidate resolutions: **8**: **21.106** (negative), **21.132** (construction for every prime), **21.121(a)** (infimum not attained at p=2), **21.68** (semi-abelian non-monomial group of order2592), **16.87(a),(b)** (nonabelian, nonperiodic variety with maximal test rank at every finite rank), **10.35** (torsion-free complex matrix group not residually rational in the same degree), **16.28(a)** (a closed set with every power greater than one nonclosed), and **17.34** (divisible subgroups have trivial dominion in each full torsion-free nilpotency quasivariety). All await outside review and further novelty checks.
- Externally reviewed new solutions: 0.
- Both the 21.26 search through order2000 and the 19.20 search through order255 are complete. The 19.20 order256 extension is running in 16 GAP workers; all process command lines and logs were just revalidated. At this snapshot 19,168 groups have been counted, with 0 equality hits and 25 reverse inequalities.
...
**17.34:** For a rational nilpotent Lie algebra L and subalgebra h, the augmentation module M=I/(U(L)h+I^(c+1)) has a cocycle d with kernel h. Its semidirect product with L stays of class at most c, giving two embeddings whose equalizer is h. Malcev completion and BCH transfer this to every divisible subgroup of a torsion-free nilpotent group. Thirteen independent tensor-presentation controls and 4,000 rational BCH identities passed. Shakhova's 2015 class-two resolution is explicitly credited; the proposed new content is the arbitrary-class result. See `research/17.34-proof.md` and `research/17.34-review.md`. Further novelty and outside review remain.
...
kept separate from the eight candidate resolutions.
...
2. Continue novelty and mathematical audits of all eight candidate resolutions.
